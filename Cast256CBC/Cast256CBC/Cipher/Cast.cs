...
namespace Cast256CBC.Cipher
{
    static class CastExtenstions
    {
        ...
        public static byte ByteAt(this uint value, int n)
        {
            return ((byte)((value) >> (8 * n)));
        }

        public static uint BitSwap(this uint value)
        {
            return (value.RotateLeft(8) & 0x00ff00ff | value.RotateRight(8) & 0xff00ff00);
        }
    ...
    static class SBox
    {
        public static readonly uint[,] values = { 
        {
    ...
    public class Cast : ICipher
    {
        ...
        public const int CM = 0x5a827999;   // 2^30 * sqrt(2)
        public const int M  = 0x6ed9eba1;   // 2^30 * sqrt(3)
        ...
        public void SetKey(uint[] inKey, int keyLenght)
        {
            ...
            // keyLenght 32, 64, 96, 128, ..., 256
            for(i = 0; i < keyLenght / 32; ++i)
                lk[i] = inKey[i].BitSwap(); //заполняем данными ключа
            ...
            for(i = 0; i < 96; i += 8) // 24 x 4
            {
                ...
                lKey[i + 0] = lk[0]; lKey[i + 1] = lk[2];
                lKey[i + 2] = lk[4]; lKey[i + 3] = lk[6];
                lKey[i + 4] = lk[7]; lKey[i + 5] = lk[5];
                lKey[i + 6] = lk[3]; lKey[i + 7] = lk[1];
            }
        }
        ...
        void F1(ref uint y, uint x,uint kr,uint km)
        {
            uint t, u;

            t = (km + x).RotateLeft((int)kr);
            u =  SBox.values[0, t.ByteIa()];
            u ^= SBox.values[1, t.ByteIb()];
            u -= SBox.values[2, t.ByteIc()];
            u += SBox.values[3, t.ByteId()];
            y ^= u;
        }

        void F2(ref uint y, uint x,uint kr,uint km)
        {
            uint t, u;
        ...
        void F3(ref uint y, uint x,uint kr,uint km)
        {
            uint t, u;
        ...
        void ForwardQuadRound(uint[] block, uint n)
        {
            F1(ref block[C], block[D], lKey[n],     lKey[n + 4]);    
            F2(ref block[B], block[C], lKey[n + 1], lKey[n + 5]);    
            F3(ref block[A], block[B], lKey[n + 2], lKey[n + 6]);
            F1(ref block[D], block[A], lKey[n + 3], lKey[n + 7]);
        }

        void ReverseQuadRound(uint[] block, uint n)                
        {
            F1(ref block[D], block[A], lKey[n + 3], lKey[n + 7]);    
            F3(ref block[A], block[B], lKey[n + 2], lKey[n + 6]);    
            F2(ref block[B], block[C], lKey[n + 1], lKey[n + 5]);
        ...
        void KeyRound(uint[] k, uint[] tr,uint[] tm)			
        {
            F1(ref k[kG], k[kH], tr[0], tm[0]);	
            F2(ref k[kF], k[kG], tr[1], tm[1]); 
            F3(ref k[kE], k[kF], tr[2], tm[2]); 
            F1(ref k[kD], k[kE], tr[3], tm[3]); 
            F2(ref k[kC], k[kD], tr[4], tm[4]); 
            F3(ref k[kB], k[kC], tr[5], tm[5]); 
            F1(ref k[kA], k[kB], tr[6], tm[6]);
            F2(ref k[kH], k[kA], tr[7], tm[7]);
        }
