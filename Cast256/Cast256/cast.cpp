...
#define Ia(t) byte(t,A)
#define Ib(t) byte(t,B)
#define Ic(t) byte(t,C)
#define Id(t) byte(t,D)
...
#define forward_quad_round(x,n)                 \
    f1(x[C],x[D],l_key[n],    l_key[n + 4]);    \
    f2(x[B],x[C],l_key[n + 1],l_key[n + 5]);    \
    f3(x[A],x[B],l_key[n + 2],l_key[n + 6]);    \
    f1(x[D],x[A],l_key[n + 3],l_key[n + 7])


#define reverse_quad_round(x, n)                \
    f1(x[D],x[A],l_key[n + 3],l_key[n + 7]);    \
    f3(x[A],x[B],l_key[n + 2],l_key[n + 6]);    \
    f2(x[B],x[C],l_key[n + 1],l_key[n + 5]);    \
    f1(x[C],x[D],l_key[n],    l_key[n + 4])
...
#define key_round(k,tr,tm)			\
    f1(k[kG], k[kH], tr[0], tm[0]);	\
	f2(k[kF], k[kG], tr[1], tm[1]); \
	f3(k[kE], k[kF], tr[2], tm[2]); \
	f1(k[kD], k[kE], tr[3], tm[3]); \
	f2(k[kC], k[kD], tr[4], tm[4]); \
	f3(k[kB], k[kC], tr[5], tm[5]); \
	f1(k[kA], k[kB], tr[6], tm[6]); \
	f2(k[kH], k[kA], tr[7], tm[7])
...
{   
    ...
    forward_quad_round(blk,  0); forward_quad_round(blk,  8);
	forward_quad_round(blk, 16); forward_quad_round(blk, 24);
	forward_quad_round(blk, 32); forward_quad_round(blk, 40);

    reverse_quad_round(blk, 48); reverse_quad_round(blk, 56);
    reverse_quad_round(blk, 64); reverse_quad_round(blk, 72);
    reverse_quad_round(blk, 80); reverse_quad_round(blk, 88);
...
{   
    ...
    forward_quad_round(blk, 88); forward_quad_round(blk, 80);
    forward_quad_round(blk, 72); forward_quad_round(blk, 64);
    forward_quad_round(blk, 56); forward_quad_round(blk, 48);

    reverse_quad_round(blk, 40); reverse_quad_round(blk, 32);
    reverse_quad_round(blk, 24); reverse_quad_round(blk, 16);
    reverse_quad_round(blk,  8); reverse_quad_round(blk,  0);
